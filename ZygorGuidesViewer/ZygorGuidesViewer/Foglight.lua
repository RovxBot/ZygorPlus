-- WotLK Foglight.  This uses the pre-Cata WorldMap overlay contract rather
-- than the retail MapCanvas provider used by the Anniversary addon.
local _, namespace = ...
...
local Foglight=ZGV:RegisterModule("Foglight",{enabled=false,installed=false,data={
	-- Eastern
	['Alterac']={
...
local function profile()
  return ZGV.db and ZGV.db.profile or {}
end

function Foglight:OverlayCount(map)
  local entries=self.data[map]; if not entries then return 0 end
  local count=0; for _ in pairs(entries) do count=count+1 end; return count
end

function Foglight:Install()
  if self.installed then return true end
  if type(_G.GetNumMapOverlays)~="function" or type(_G.GetMapOverlayInfo)~="function" then return false end
  self.installed=true
  self.PreGetNumMapOverlays=_G.GetNumMapOverlays
  self.PreGetMapOverlayInfo=_G.GetMapOverlayInfo
  _G.GetNumMapOverlays=function()
    if not Foglight.enabled then return Foglight.PreGetNumMapOverlays() end
    local map=type(GetMapInfo)=="function" and GetMapInfo()
    return Foglight:OverlayCount(map)
  end
  _G.GetMapOverlayInfo=function(index)
    if not Foglight.enabled then return Foglight.PreGetMapOverlayInfo(index) end
    local map=type(GetMapInfo)=="function" and GetMapInfo()
    local overlays=map and Foglight.data[map]
    if not overlays then return nil,0,0,0,0,0,0 end
    local names={}; for name in pairs(overlays) do names[#names+1]=name end; table.sort(names)
    local name=names[tonumber(index) or 0]; local values=name and overlays[name]
    if not values then return nil,0,0,0,0,0,0 end
    return "Interface\\WorldMap\\"..map.."\\"..name,values[1],values[2],values[3],values[4],0,0,"zygfog"
  end
  return true
end

function Foglight:Refresh()
  if type(WorldMapFrame_Update)=="function" and WorldMapFrame and WorldMapFrame:IsShown() then WorldMapFrame_Update() end
end
function Foglight:ShowOverlay()
  if not self:Install() then return false end
  self.enabled=true; self:Refresh(); return true
end
function Foglight:HideOverlay() self.enabled=false; self:Refresh(); return true end
function Foglight:ToggleOverlay()
  local desired=not self.enabled
  if ZGV.db and ZGV.db.profile then ZGV.db.profile.foglight=desired end
  return desired and self:ShowOverlay() or self:HideOverlay()
end
function Foglight:TurnOff() return self:HideOverlay() end
function Foglight:DebugMap()
  local map=type(GetMapInfo)=="function" and GetMapInfo() or "unknown"
  local message=string.format("Foglight: %s, %d overlay%s.",tostring(map),self:OverlayCount(map),self:OverlayCount(map)==1 and "" or "s")
  if ZGV.Print then ZGV:Print(message) end
  return message
end
function Foglight:OnStartup() if profile().foglight then self:ShowOverlay() end end
