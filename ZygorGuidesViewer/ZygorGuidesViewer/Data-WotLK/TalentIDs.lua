-- Locale-independent talent IDs for the 3.3.5a (build 12340) talent trees.
-- Source: Zgor 3.3.5a/ZygorTalentAdvisor/Registering.lua (legacy Talent Advisor 2.0.1293).
-- The source table stores Talent.dbc IDs (the IDs emitted by GetTalentLink),
-- not localized spell names.  Normalize the authored English aliases once so
-- legacy text builds can resolve through the live client's localized trees.
local ZGV = ZygorGuidesViewer
if not (ZGV and ZGV.Data and ZGV.Data.Register) then return end
...
local function normalize(value)
  value=tostring(value or ""):lower():gsub("|r",""):gsub("|c%x%x%x%x%x%x%x%x","")
  return value:gsub("^%s+",""):gsub("%s+$",""):gsub("%s+"," ")
end

local talentIDs={}
for class,names in pairs(sourceIDs) do
  local classKey=tostring(class):upper()
  if classKey=="CUNNING" or classKey=="FEROCITY" or classKey=="TENACITY" then
    classKey="PET "..classKey
  end
  local normalized={}
  for talentName,talentID in pairs(names) do
    if talentName~="pet" and type(talentID)=="number" then
      normalized[normalize(talentName)]=talentID
    end
  end
  talentIDs[classKey]=normalized
end

ZGV.Data:Register("talentIDs",1,talentIDs,{
  client="3.3.5a.12340",
  source="legacy Talent Advisor 2.0.1293 TalentsToNumbers (Talent.dbc IDs)",
  sourcePath="Zgor 3.3.5a/ZygorTalentAdvisor/Registering.lua:172",
})
